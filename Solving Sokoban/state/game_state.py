...
class GameState:
  ...
  def __init__(self, map_file = None, prev_state = None):
    ...
    # Otherwise, load it from scratch using the given file
      ...
      self._walls, self._player_position, self._boxes, self._switches, self._armory_points, self._mice, self._r_enemies, self._c_enemies = load_state(map_file)
  ...
  def __str__(self):
    info = "#---------INFO-START---------#\n" \
           ...
           "#---------INFO-END-----------#\n".format(self._boxes, self._switches, self._armory_points,
                                                     self.get_enemies(), self._player_position, self._mice, self._score)
    return info
  ...
  def __hash__(self):
    val = hash(tuple(self._boxes)) + hash(tuple(self._switches)) \
          + hash(self._player_position) + hash(tuple(self.get_enemies())) \
          + hash(self._score) + hash(tuple(self._mice))
    return val
